...
#import "SaKaStatusBarNotification.h"
...
@implementation SaKaScrollLabel
{
...
@end
@implementation SaKaWindowContainer
...
@interface SaKaStatusBarNotification()
...
@implementation SaKaStatusBarNotification
...
- (SaKaStatusBarNotification *)init
{
    self = [super init];
    if (self) {
        self.notificationLabelBackgroundColor = [UIColor colorWithRed:227/255.f green:181/255.f blue:117/255.f alpha:1.0];
        ...
        self.notificationStyle = SaKaNotificationStyleStatusBarNotification;
        self.notificationAnimationInStyle = SaKaNotificationAnimationStyleBottom;
        self.notificationAnimationOutStyle = SaKaNotificationAnimationStyleBottom;
        self.notificationAnimationType = SaKaNotificationAnimationTypeOverlay;
        [self setValue:@0 forKey:NSStringFromSelector(@selector(notificationIsDismissing))];
...
- (CGFloat)getNotificationLabelHeight
{
    switch (self.notificationStyle) {
        case SaKaNotificationStyleStatusBarNotification:
            return [self getStatusBarHeight];
        case SaKaNotificationStyleNavigationBarNotification:
...
- (void)setupNotificationView:(UIView *)view
{
    ...
    switch (self.notificationAnimationInStyle) {
        case SaKaNotificationAnimationStyleTop:
            view.frame = [self getNotificationLabelTopFrame];
            break;
        case SaKaNotificationAnimationStyleBottom:
            view.frame = [self getNotificationLabelBottomFrame];
            break;
        case SaKaNotificationAnimationStyleLeft:
            view.frame = [self getNotificationLabelLeftFrame];
            break;
        case SaKaNotificationAnimationStyleRight:
...
{
    self.notificationLabel = [SaKaScrollLabel new];
...
{
    self.notificationWindow = [[SaKaWindowContainer alloc] initWithFrame:[[[UIApplication sharedApplication] keyWindow] bounds]];
    ...
    SaKaViewController *rootViewController = [[SaKaViewController alloc] init];
...
- (void)createStatusBarView
{
    ...
    if (self.notificationAnimationType == SaKaNotificationAnimationTypeReplace) {
...
- (void)displayNotificationWithMessage:(NSString *)message completion:(void (^)(void))completion
{
    if (!self.notificationIsShowing) {
        self.isCustomView = NO;
        [self setValue:@1 forKey:NSStringFromSelector(@selector(notificationIsShowing))];
...
- (void)dismissNotificationWithCompletion:(void (^)(void))completion
{
    if (self.notificationIsShowing) {
        ...
        [self setValue:@1 forKey:NSStringFromSelector(@selector(notificationIsDismissing))];
        ...
        [UIView animateWithDuration:self.notificationAnimationDuration animations:^{
            ...
            view = nil;
            [self setValue:@0 forKey:NSStringFromSelector(@selector(notificationIsShowing))];
            [self setValue:@0 forKey:NSStringFromSelector(@selector(notificationIsDismissing))];
